...
import { User, AlertTriangle, Star, Eye } from "lucide-react";
    ...
    <div className="bg-white dark:bg-slate-700 rounded-lg p-4 shadow-md">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {/* Spoiler Badge */}
          {isSpoiler && (
            <span className="px-2 py-1 bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400 text-xs rounded-full font-medium flex items-center gap-1">
        ...
        {shouldBlur && (
          <div className="absolute inset-0 flex items-center justify-center bg-white/30 dark:bg-slate-700/30 backdrop-blur-[1px] rounded">
            <button
              ...
              className="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white text-sm font-medium rounded-lg shadow-md transition-colors flex items-center gap-2"
            >
              <Eye size={18} /> Show Content
            </button>
          </div>
        )}
      </div>
